...
age_listToTest = np.array([])
salary_listToTest = np.array([])

with open('Football_players.csv', encoding='latin-1') as f:
    csv_list = list(csv.reader(f))

for row in csv_list[1:81]:
    age_list = np.append(age_list, int(row[4]))
    salary_list = np.append(salary_list, int(row[8]))

for row in csv_list[81:]:
    age_listToTest = np.append(age_listToTest, int(row[4]))
    salary_listToTest = np.append(salary_listToTest, int(row[8]))
...
def simlin_plot(x, y, b1, b0):
    ...
    plt.scatter(x, ploty, color='r')
    plt.ylabel("Salary")
    plt.xlabel("Age")
    plt.title("Simple Linear Regression: Age vs Salary")
    plt.show()

simlin_plot(age_listToTest, salary_listToTest, b1, b0)